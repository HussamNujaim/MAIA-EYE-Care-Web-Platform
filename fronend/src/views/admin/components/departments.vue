...
<div class="app-container">
       <div class="filter-container">
        <span>
          ...
          <el-input
            ...
            @input="search"
            v-model="search_deptname"
          ></el-input>
        </span>
        <el-button type="primary" plain  icon="el-icon-search"></el-button>
        <el-button  type="primary" @click="handleCreate" icon="el-icon-plus">New Department</el-button>
    </div>
    <div class="cont">
      <el-table
      :data="tableData"
      ...
      class="table-check"
    >
      <el-table-column prop="id" label="Department id"></el-table-column>
      <el-table-column prop="deptName" label="Department Name"></el-table-column>
      <el-table-column prop="notes" label="Description"></el-table-column>
   ...
   <el-pagination
        ...
        >
          ...
          >
            <el-input style="width:25rem" v-model="addForm.deptName" autocomplete="off"></el-input>
          </el-form-item>
          <br />
          <el-form-item
            label="Description"
          ...
          >
            <el-input style="width:25rem" v-model="addForm.notes" autocomplete="off"></el-input>
          </el-form-item>
...
export default {
  ...
  data() {
    return {
      urlCreate:'/api/dr-department/insert',
      urlUpdate:'/api/dr-department/update',
      urlDelete:'/api/dr-department/delete',
      urlGet:'/api/dr-department/get',
      tableData:[],
    search_deptname:"",
    pagesize:5,
    currentPage:1,
...
// this.$router.go(0);
      ...
      this.$refs['addForm'].validate((valid) => {
        if (valid) {
          this.addForm.remark=res.obj // get product's image url after being uploaded to the server 
          if(this.dialogStatus==='create')
          {

            getUser({username:sessionStorage.getItem("username")}).then(res3=>
            {
             console.log("sss" + res3.userId)
               createData(this.urlCreate, this.addForm).then(newresponse => {
            this.dialogFormVisible = false
            this.$notify({
              title: 'Success',
...
//              this.$router.go(0);
  ...
  search(val){
    console.log(val)
    this.search_deptname=val;
    this.tableData = this.table.filter(e =>
        e.title.match(this.search_deptname)
      );
  },
   btntag(msg) {
